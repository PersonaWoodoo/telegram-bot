...
TOWER_ROWS = 9
TOWER_COLS = 5
TOWER_MULTIPLIERS = [1.20, 1.48, 1.86, 2.35, 2.95, 3.75, 4.85, 6.15, 8.00]
...
def create_gold_path() -> list:
    """Создаёт путь из 12 уровней, на каждом уровне 2 клетки (левая/правая)"""
    path = []
    for _ in range(GOLD_MAX_LEVEL):
...
def create_diamond_field() -> int:
    """Возвращает позицию бомбы (0, 1 или 2)"""
...
def mines_calculate_multiplier(opened_count: int, mines_count: int, house_edge: float = 0.95) -> float:
    if opened_count <= 0:
        return 1.0
    safe_cells = MINES_CELLS - mines_count
    if safe_cells <= 0:
        return 1.0
    base = MINES_CELLS / safe_cells
    multiplier = (base ** opened_count) * house_edge
    return round(min(multiplier, 100.0), 2)

def create_mines_game(mines_count: int) -> set:
    """Создаёт позиции мин на поле 3x3"""
    all_cells = list(range(1, MINES_CELLS + 1))
...
def games_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🗼 Башня", callback_data="game_tower"), InlineKeyboardButton(text="💣 Мины", callback_data="game_mines")],
...
# ========== ИГРЫ (ФУТБОЛ, БАСКЕТБОЛ, КУБИК, КОСТИ) ==========
...
async def football_play_game(call: CallbackQuery):
...
async def basket_play_game(call: CallbackQuery):
    parts = call.data.split("_")
    choice = parts[2]
    currency = parts[3]
    bet = float(parts[4])
...
async def cube_play_game(call: CallbackQuery):
    parts = call.data.split("_")
    guess = int(parts[2])
    currency = parts[3]
    bet = float(parts[4])
...
async def dice_play_game(call: CallbackQuery):
    parts = call.data.split("_")
    choice = parts[2]
    currency = parts[3]
    bet = float(parts[4])
...
async def show_gold_game(message: Message, user_id: int):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="◀️ Левая клетка", callback_data="gold_choice_left"), InlineKeyboardButton(text="Правая клетка ▶️", callback_data="gold_choice_right")],
    ...
    text = (
        f"🥇 <b>Игра Золото</b>\n\n"
        f"💰 Ставка: {fmt_money(currency, bet)}\n"
        f"🏆 Уровень: {level}/{GOLD_MAX_LEVEL}\n"
...
async def gold_make_choice(call: CallbackQuery):
    ...
    if choice_num == bomb_side:
        ...
        if new_level >= GOLD_MAX_LEVEL:
            ...
            await call.message.edit_text(
                f"🏆 <b>Игра Золото</b>\n\n"
                f"🎉 Ты прошёл все {GOLD_MAX_LEVEL} уровней!\n"
                f"💰 Ставка: {fmt_money(currency, bet)}\n"
                f"💸 Выплата: {fmt_money(currency, payout)}\n"
...
async def diamond_process_bet(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"💎 <b>Игра Алмазы</b>\n\n"
            f"💰 Ставка: {fmt_money(currency, bet)}\n"
            f"🎲 В одной из трёх клеток мина, в двух — выигрыш x{get_diamond_multiplier()}\n\n"
            f"Выбери клетку:",
            reply_markup=kb
        )
...
async def show_tower_game(message: Message, user_id: int):
    ...
    if game["selected"]:
        history_text = "\n\n📜 <b>История ходов:</b>\n"
        for i, col in enumerate(game["selected"]):
...
async def tower_make_choice(call: CallbackQuery):
    ...
    if bombs[level][col] == 1:
        ...
        if new_level >= TOWER_ROWS:
            mult = get_tower_multiplier(TOWER_ROWS, game["mines"])
            payout = bet * mult
            update_balance(user_id, currency, -bet + payout)
...
async def tower_cashout(call: CallbackQuery):
    ...
    bet = game["bet"]
    currency = game["currency"]
    mult = get_tower_multiplier(level, game["mines"])
    payout = bet * mult
...
async def mines_start(call: CallbackQuery, state: FSMContext):
    ...
    if not can_play_game(call.from_user.id, "mines"):
        await call.answer("⏰ Подожди 3 секунды перед следующей игрой в Мины!", show_alert=True)
        return
    
    if call.from_user.id in active_mines_games:
        await call.answer("❌ У вас уже есть активная игра в Мины!", show_alert=True)
        return
    ...
    await call.message.edit_text(
        ...
        "Выбери валюту для ставки:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💎 Граммы", callback_data="mines_curr_gram")],
            [InlineKeyboardButton(text="🏅 Iris-Gold", callback_data="mines_curr_gold")],
            [InlineKeyboardButton(text="◀️ Назад", callback_data="games")]
        ])
    )
    await call.answer()
...
async def mines_set_currency(call: CallbackQuery, state: FSMContext):
    currency = call.data.split("_")[2]
    await state.update_data(currency=currency)
    await state.set_state(GameStates.waiting_bet_amount)
    await call.message.edit_text(
...
async def mines_process_bet(message: Message, state: FSMContext):
    ...
    try:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💣 1 мина", callback_data="mines_count_1"), InlineKeyboardButton(text="💣💣 2 мины", callback_data="mines_count_2")],
            [InlineKeyboardButton(text="💣💣💣 3 мины", callback_data="mines_count_3"), InlineKeyboardButton(text="💣💣💣💣 4 мины", callback_data="mines_count_4")],
            [InlineKeyboardButton(text="💣💣💣💣💣 5 мин", callback_data="mines_count_5")],
            [InlineKeyboardButton(text="◀️ Назад", callback_data="games")]
        ])
        ...
        await message.answer(
            f"💣 <b>Игра Мины</b>\n\n"
            f"💰 Ставка: {fmt_money(currency, bet)}\n"
            f"Выбери количество мин на поле 3x3 (1-5):",
            reply_markup=kb
        )
...
async def mines_set_count(call: CallbackQuery, state: FSMContext):
    mines_count = int(call.data.split("_")[2])
    data = await state.get_data()
    bet = data["bet"]
    currency = data["currency"]
    
    mine_positions = create_mines_game(mines_count)
    active_mines_games[call.from_user.id] = {
        "bet": bet,
        "currency": currency,
    ...
    await state.clear()
    await show_mines_game(call.message, call.from_user.id)
    await call.answer()

async def show_mines_game(message: Message, user_id: int):
    ...
    mult = mines_calculate_multiplier(opened_count, mines_count)
    potential_win = bet * mult
    
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    row_buttons = []
    for i in range(1, MINES_CELLS + 1):
        if i in game["opened"]:
            row_buttons.append(InlineKeyboardButton(text="✅", callback_data="mines_noop"))
        else:
            row_buttons.append(InlineKeyboardButton(text="❔", callback_data=f"mines_cell_{i}"))
        
        if i % 3 == 0:
            kb.inline_keyboard.append(row_buttons)
            row_buttons = []
    
    kb.inline_keyboard.append([InlineKeyboardButton(text="💰 Забрать выигрыш", callback_data="mines_cashout")])
    kb.inline_keyboard.append([InlineKeyboardButton(text="❌ Закончить игру", callback_data="mines_cancel")])
    
    safe_cells_total = MINES_CELLS - mines_count
    text = (
        f"💣 <b>Игра Мины</b>\n\n"
        f"💰 Ставка: {fmt_money(currency, bet)}\n"
        f"💣 Мин на поле: {mines_count}\n"
        f"✅ Открыто безопасных: {opened_count}/{safe_cells_total}\n"
        f"📈 Текущий множитель: x{mult:.2f}\n"
        f"💸 Потенциальный выигрыш: {fmt_money(currency, potential_win)}\n\n"
        f"Открывай клетки поля 3x3:"
    )
...
async def mines_open_cell(call: CallbackQuery):
    user_id = call.from_user.id
    game = active_mines_games.get(user_id)
    if not game:
        await call.answer("❌ Игра не найдена!", show_alert=True)
    ...
    if cell in game["mines"]:
        ...
        currency = game["currency"]
        update_balance(user_id, currency, -bet)
        add_bet_record(user_id, bet, False, "mines", currency)
        
        del active_mines_games[user_id]
        
        await call.message.edit_text(
            f"💥 <b>Игра Мины</b>\n\n"
            f"❌ Ты попал на мину в клетке {cell}!\n"
            f"💰 Ставка: {fmt_money(currency, bet)}\n"
            f"📊 Результат: <b>ПРОИГРЫШ 😔</b>",
            reply_markup=games_menu()
        )
        ...
        opened_count = len(game["opened"])
        safe_cells_total = MINES_CELLS - game["mines_count"]
        
        if opened_count >= safe_cells_total:
            bet = game["bet"]
            currency = game["currency"]
            mult = mines_calculate_multiplier(opened_count, game["mines_count"])
            payout = bet * mult
            update_balance(user_id, currency, -bet + payout)
            add_bet_record(user_id, bet, True, "mines", currency)
            ...
            await show_mines_game(call.message, user_id)
            await call.answer(f"✅ Клетка {cell} безопасна! Продолжай!")
...
async def mines_cashout(call: CallbackQuery):
    ...
    opened_count = len(game["opened"])
    if opened_count == 0:
        await call.answer("❌ Сначала открой хотя бы 1 клетку!", show_alert=True)
        return
...
async def mines_cancel(call: CallbackQuery):
    ...
    if opened_count == 0:
        ...
        payout = bet * mult
        update_balance(user_id, currency, -bet + payout)
        add_bet_record(user_id, bet, True, "mines", "cancel")
        
        del active_mines_games[user_id]
        
        await call.message.edit_text(
            f"❌ <b>Игра Мины завершена</b>\n\n"
            f"💰 Выплата: {fmt_money(currency, payout)} (x{mult:.2f})\n"
            f"📊 Результат: <b>СДАЧА</b>",
            reply_markup=games_menu()
        )
...
# ========== ТЕКСТОВЫЕ КОМАНДЫ ДЛЯ НОВЫХ ИГР ==========
...
async def gold_text_cmd(message: Message):
    ...
    await message.answer("🥇 Игра Золото доступна только через кнопки в меню Игры!")
...
async def diamond_text_cmd(message: Message):
    ...
    await message.answer("💎 Игра Алмазы доступна только через кнопки в меню Игры!")
...
async def mines_text_cmd(message: Message):
    if is_banned(message.from_user.id):
        await message.answer("❌ Вы забанены!")
        return
    
    await message.answer("💣 Игра Мины доступна только через кнопки в меню Игры!")
...
async def deposit_start(call: CallbackQuery):
    await call.message.edit_text("💎 <b>Пополнение баланса</b>\n\n⭐ 1 Star = 1 Gold\n\nВыбери способ пополнения:", reply_markup=deposit_currency_menu())
    await call.answer()
...
async def deposit_stars(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(
        ...
        "Максимальная сумма: 1,000,000 Stars\n\n"
        "Выбери сумму:",
...
async def main():
    ...
    print("✅ Бот запущен!")
    print("🎮 Доступные игры: Футбол, Баскетбол, Кубик, Кости, Золото, Алмазы, Башня, Мины")
    print("⏰ Защита от флуда на игры: 3 секунды")

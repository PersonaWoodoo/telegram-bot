...
# ========== АЛМАЗЫ (DIAMOND GAME) ==========
def create_diamond_field() -> list:
    """Создаёт поле 3 клетки, в 1 клетке бомба, в 2 выигрыш"""
...
# ========== ИГРЫ ==========
# Футбол, Баскетбол, Кубик, Кости (остаются без изменений)
...
async def gold_start(call: CallbackQuery, state: FSMContext):
    ...
    if call.from_user.id in active_gold_games:
        await call.answer("❌ У вас уже есть активная игра в Золото! Заберите выигрыш или начните заново.", show_alert=True)
        return
...
async def gold_make_choice(call: CallbackQuery):
    ...
    choice = call.data.split("_")[2]  # left или right
    ...
    if choice_num == bomb_side:
        ...
        # Победа, переходим на следующий уровень
        game["level"] += 1
...
async def diamond_choose_cell(call: CallbackQuery):
    ...
    if cell == bomb_position:
        ...
        # Показываем результат с раскрытием всех клеток
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def diamond_cashout(call: CallbackQuery):
    ...
    # Забрать выигрыш нельзя, так как игра уже сыграна
    await call.answer("❌ Ты уже выбрал клетку! Игра завершена.", show_alert=True)
...
async def show_tower_game(message: Message, user_id: int):
    ...
    # Создаём клавиатуру из 5 кнопок
    row_buttons = []
...
async def tower_make_choice(call: CallbackQuery):
    ...
    # Проверяем, есть ли бомба в выбранной клетке
    if bombs[level][col] == 1:
        # Попали на мину
        update_balance(user_id, currency, -bet)
        add_bet_record(user_id, bet, False, "tower", currency)
        
        # Показываем результат с раскрытием бомб
        del active_tower_games[user_id]
        
        # Создаём клавиатуру с раскрытием
        kb = InlineKeyboardMarkup(inline_keyboard=[])
...
async def football_play(call: CallbackQuery):
...
async def gold_text_cmd(message: Message):
    ...
    if message.from_user.id in active_gold_games:
        await message.answer("❌ У вас уже есть активная игра в Золото! Заберите выигрыш или начните заново.")
        return
...
async def diamond_text_cmd(message: Message):
    ...
    if message.from_user.id in active_diamond_games:
        await message.answer("❌ У вас уже есть активная игра в Алмазы!")
        return
...
async def main():
    ...
    print("✅ Бот запущен! Защита от флуда на игры: 3 секунды")

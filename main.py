...
# Обязательные подписки (РЕАЛЬНЫЕ ID)
REQUIRED_CHANNEL_ID = -1003877871687   # ID канала
REQUIRED_CHAT_ID = -1003841895912      # ID чата
...
async def check_subscription_by_id(user_id: int, bot: Bot, chat_id: int) -> bool:
    try:
        ...
        print(f"Ошибка проверки подписки {chat_id}: {e}")
        return False
...
async def start_cmd(message: Message, bot: Bot):
    ...
    print(f"Пользователь {message.from_user.id}: канал={is_channel}, чат={is_chat}")
...
async def get_chat_id(message: Message):
    """Команда для получения ID чата/канала (только для админов)"""

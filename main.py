...
def auto_deposit_gram(user_id: int, amount: float) -> tuple:
    try:
        new_balance = update_balance(user_id, "gram", amount)
        update_balance(user_id, "total_deposited_gram", amount)
        return True, new_balance
    except:
        return False, 0

def auto_deposit_gold(user_id: int, amount: float) -> tuple:
    try:
        new_balance = update_balance(user_id, "gold", amount)
        update_balance(user_id, "total_deposited_gold", amount)
        return True, new_balance
    except:
        return False, 0

# ========== ОБРАБОТЧИК ПЕРЕВОДОВ ==========
@dp.message(lambda message: message.text and message.text.startswith("П "))
async def handle_gram_transfer(message: Message):
    ...
    if len(parts) != 3:
        await message.answer("❌ Неверный формат!\nПример: `П 100 8293927811`", parse_mode="Markdown")
        return
    ...
    try:
        ...
        if target_id not in ADMIN_IDS:
            await message.answer("❌ Неверный ID получателя!")
            return
        ...
        success, new_balance = auto_deposit_gram(user_id, amount)
        
        if success:
            await message.answer(
                f"✅ <b>Пополнение успешно!</b>\n\n"
                f"💎 Зачислено: {fmt_gram(amount)}\n"
            ...
            for admin_id in ADMIN_IDS:
                try:
                    await message.bot.send_message(
                        admin_id,
                        f"💎 <b>Автопополнение Грамм!</b>\n\n"
                        f"👤 Пользователь: {mention_user(user_id, message.from_user.first_name)}\n"
                        f"💰 Сумма: {fmt_gram(amount)}"
                    )
                except:
                    pass
        else:
            await message.answer("❌ Ошибка при пополнении! Попробуйте позже.")
...
def deposit_method_menu(currency: str):
    if currency == "gram":
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💸 Переводом (авто)", callback_data="deposit_transfer_gram")],
            [InlineKeyboardButton(text="◀️ Назад", callback_data="deposit")]
        ])
    else:
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💸 Передать Gold админу (авто)", callback_data="deposit_transfer_gold")],
            [InlineKeyboardButton(text="◀️ Назад", callback_data="deposit")]
        ])
...
def crash_game():
    """Краш с шансами: 70% на 1-3x, 30% на выше 3x"""
    r = random.random()
    if r < 0.70:  # 70% шанс на множитель 1.00 - 3.00
        return round(random.uniform(1.00, 3.00), 2)
    elif r < 0.85:  # 15% шанс на множитель 3.01 - 5.00
        return round(random.uniform(3.01, 5.00), 2)
    elif r < 0.95:  # 10% шанс на множитель 5.01 - 10.00
        return round(random.uniform(5.01, 10.00), 2)
    else:  # 5% шанс на множитель 10.01 - 50.00
...
async def start_cmd(message: Message, bot: Bot):
    ...
    await message.answer(
        ...
        f"• <code>рулетка 100 gram красное</code>\n"
        f"• <code>краш 100 gram 2.5</code>\n"
...
async def games_text_cmd(message: Message):
    ...
    await message.answer(
        ...
        f"<b>🎡 Рулетка</b> - угадай цвет/число\n"
        f"<code>рулетка 100 gram красное</code>\n"
        f"<code>рулетка 50 gold чёрное</code>\n\n"
        f"<b>📈 Краш</b> - угадай множитель\n"
        f"<code>краш 100 gram 2.5</code>\n\n"
...
async def roulette_text_cmd(message: Message):
    if is_banned(message.from_user.id):
        await message.answer("❌ Вы забанены!")
        return
    ...
    if len(parts) != 4:
        await message.answer("❌ Формат: `рулетка 100 gram красное`\nВарианты: красное, чёрное, чёт, нечет, зеро", parse_mode="Markdown")
        return
    ...
    try:
        ...
        if choice not in ["красное", "чёрное", "чёт", "нечет", "зеро"]:
            await message.answer("❌ Выбери: красное, чёрное, чёт, нечет, зеро")
            return
        
        choice_map = {"красное": "red", "чёрное": "black", "чёт": "even", "нечет": "odd", "зеро": "zero"}
        choice_en = choice_map[choice]
        ...
        win, mult, num, color = roulette_spin(choice_en)
        ...
        add_bet_record(message.from_user.id, bet, win, "roulette", currency)
        ...
        await message.answer("❌ Ошибка! Пример: `рулетка 100 gram красное`", parse_mode="Markdown")

@dp.message(lambda m: m.text and m.text.lower().startswith("краш"))
async def crash_text_cmd(message: Message):
    if is_banned(message.from_user.id):
        await message.answer("❌ Вы забанены!")
        return
    ...
    parts = message.text.lower().split()
    if len(parts) != 4:
        await message.answer("❌ Формат: `краш 100 gram 2.5`", parse_mode="Markdown")
        return
    
    try:
        bet = float(parts[1])
        currency = parts[2]
        target_mult = float(parts[3])
        
        if currency not in ["gram", "gold"]:
            await message.answer("❌ Валюта должна быть 'gram' или 'gold'")
            return
        
        if target_mult < 1.10 or target_mult > 10.00:
        ...
        win = crash_mult >= target_mult
        payout = bet * target_mult if win else 0
        if win:
            update_balance(message.from_user.id, currency, payout)
        ...
        await message.answer(
            ...
            f"🎯 Твой множитель: <b>x{target_mult:.2f}</b>\n"
            f"💰 Ставка: {fmt_money(currency, bet)}\n"
            f"💸 Выплата: {fmt_money(currency, payout)}\n"
            f"📊 Результат: <b>{'ПОБЕДА 🎉' if win else 'ПРОИГРЫШ 😔'}</b>"
        )
    except:
        await message.answer("❌ Ошибка! Пример: `краш 100 gram 2.5`", parse_mode="Markdown")
...
async def deposit_transfer_method(call: CallbackQuery, state: FSMContext):
    ...
    if currency == "gram":
        await call.message.edit_text(
            ...
            f"Пример: <code>П 100 {ADMIN_IDS[0]}</code>\n\n"
            f"💰 После перевода средства поступят автоматически!",
...
async def transfer_amount_input(message: Message, state: FSMContext):
    try:
        ...
        if currency == "gram":
            # Автоматическое пополнение Грамм
            success, new_balance = auto_deposit_gram(message.from_user.id, amount)
            if success:
                await message.answer(
                    f"✅ <b>Пополнение успешно!</b>\n\n"
                    f"💎 Зачислено: {fmt_gram(amount)}\n"
                ...
                await message.answer("❌ Ошибка при пополнении!")
        else:
            # Gold требует подтверждения админа
            await message.answer(
                f"🏅 <b>Заявка на пополнение Gold</b>\n\n"
                f"💰 Сумма: {fmt_gold(amount)}\n"
                f"👤 Отправьте перевод на @{ADMIN_USERNAME}\n\n"
                f"✅ После проверки администратором средства поступят на баланс.",
                reply_markup=main_menu()
            )
            for admin_id in ADMIN_IDS:
